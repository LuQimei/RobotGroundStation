...
const char* GCMapPolyline::jsonPolylineKey = "polyline";

GCMapPolyline::GCMapPolyline(QObject* parent)
    : QObject               (parent)
    , _dirty                (false)
    , _interactive          (false)
    , _resetActive          (false)
...
GCMapPolyline::GCMapPolyline(const GCMapPolyline& other, QObject* parent)
    : QObject               (parent)
    , _dirty                (false)
    , _interactive          (false)
    , _resetActive          (false)
...
const GCMapPolyline& GCMapPolyline::operator=(const GCMapPolyline& other)
{
    clear();
...
void GCMapPolyline::_init(void)
{
    connect(&_polylineModel, &QmlObjectListModel::dirtyChanged, this, &GCMapPolyline::_polylineModelDirtyChanged);
    connect(&_polylineModel, &QmlObjectListModel::countChanged, this, &GCMapPolyline::_polylineModelCountChanged);

    connect(this, &GCMapPolyline::countChanged, this, &GCMapPolyline::isValidChanged);
    connect(this, &GCMapPolyline::countChanged, this, &GCMapPolyline::isEmptyChanged);
}

void GCMapPolyline::clear(void)
{
    _polylinePath.clear();
    emit pathChanged();
...
void GCMapPolyline::adjustVertex(int vertexIndex, const QGeoCoordinate coordinate, bool isEmit)
{
    _polylinePath[vertexIndex] = QVariant::fromValue(coordinate);
    if(isEmit){
        emit pathChanged();
    }
    _polylineModel.value<GCQGeoCoordinate*>(vertexIndex)->setCoordinate(coordinate);
    setDirty(true);
}

void GCMapPolyline::setDirty(bool dirty)
{
    if (_dirty != dirty) {
        _dirty = dirty;
        if (!dirty) {
...
QGeoCoordinate GCMapPolyline::_coordFromPointF(const QPointF& point) const
{
...
QPointF GCMapPolyline::_pointFFromCoord(const QGeoCoordinate& coordinate) const
{
    if (_polylinePath.count() > 0) {
        double y, x, down;
        QGeoCoordinate tangentOrigin = _polylinePath[0].value<QGeoCoordinate>();
...
void GCMapPolyline::setPath(const QList<QGeoCoordinate>& path)
{
    ...
    for (const QGeoCoordinate& coord: path) {
        ...
        _polylineModel.append(new GCQGeoCoordinate(coord, this));
    }
...
void GCMapPolyline::setPath(const QVariantList& path)
{
    _beginResetIfNotActive();
    ...
    for (int i=0; i<_polylinePath.count(); i++) {
        _polylineModel.append(new GCQGeoCoordinate(_polylinePath[i].value<QGeoCoordinate>(), this));
    }
    setDirty(true);
...
QList<QGeoCoordinate> GCMapPolyline::coordinateList(void) const
{
...
void GCMapPolyline::splitSegment(int vertexIndex)
{
    ...
    if (nextIndex == 0) {
        ...
        _polylineModel.insert(nextIndex, new GCQGeoCoordinate(newVertex, this));
        _polylinePath.insert(nextIndex, QVariant::fromValue(newVertex));
        emit pathChanged();
    }
}

void GCMapPolyline::appendVertex(const QGeoCoordinate& coordinate)
{
    _polylinePath.append(QVariant::fromValue(coordinate));
    _polylineModel.append(new GCQGeoCoordinate(coordinate, this));
    emit pathChanged();
}

void GCMapPolyline::removeVertex(int vertexIndex)
{
...
void GCMapPolyline::setInteractive(bool interactive)
{
...
QGeoCoordinate GCMapPolyline::vertexCoordinate(int vertex) const
{
    if (vertex >= 0 && vertex < _polylinePath.count()) {
        return _polylinePath[vertex].value<QGeoCoordinate>();
    } else {
        qWarning() << "GCMapPolyline::vertexCoordinate bad vertex requested";
        return QGeoCoordinate();
    }
}

QList<QPointF> GCMapPolyline::nedPolyline(void)
{
...
QList<QGeoCoordinate> GCMapPolyline::offsetPolyline(double distance)
{
...
void GCMapPolyline::_polylineModelDirtyChanged(bool dirty)
{
    if (dirty) {
        setDirty(true);
    }
...
void GCMapPolyline::_polylineModelCountChanged(int count)
{
    emit countChanged(count);
}


double GCMapPolyline::length(void) const
{
    double length = 0;
...
void GCMapPolyline::appendVertices(const QList<QGeoCoordinate>& coordinates)
{
    _beginResetIfNotActive();

    QList<QObject*> objects;
    for (const QGeoCoordinate& coordinate: coordinates) {
        objects.append(new GCQGeoCoordinate(coordinate, this));
        _polylinePath.append(QVariant::fromValue(coordinate));
    }
...
void GCMapPolyline::beginReset(void)
{
    _resetActive = true;
    _polylineModel.beginReset();
}

void GCMapPolyline::endReset(void)
{
    _resetActive = false;
...
void GCMapPolyline::_beginResetIfNotActive(void)
{
    if (!_resetActive) {
        beginReset();
    }
...
void GCMapPolyline::_endResetIfNotActive(void)
{
    if (!_resetActive) {
        endReset();
    }
...
void GCMapPolyline::setTraceMode(bool traceMode)
{
...
void GCMapPolyline::selectVertex(int index)
{
...
//                    << QString("GCMapPolyline: Selected vertex index (%1) is out of bounds! "
